...
export default function Report() {
  ...
  const generateProductReport = (product: Product) => (
    <div className="p-4">
      ...
      <img src={productImages[product.name]} alt={product.name} className="product-image w-48 h-48 object-cover mb-4 rounded-lg shadow" />
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {products.map((p) => (
          <button key={p.name} onClick={() => handleReportClick(p.name)} className="card rounded-lg p-4 text-center hover:shadow-lg transition-shadow">
            <div className="w-full h-32 mb-2 flex items-center justify-center">
              <img src={productImages[p.name]} alt={p.name} className="product-image w-full h-full object-cover rounded-lg" />
            </div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100">{p.name}</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400">Stock: {p.stock}%</p>
          </button>
        ))}
